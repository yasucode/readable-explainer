...
もし英語のテキストであれば、日本語訳も提供してください。ただし、単語や専門用語の場合は訳さずにそのままにしてください。
...
学習者がこのテキストを完全に理解し、長期記憶に定着させることを目標とした解説をお願いします。`;
  ...
  if (onChunk) {
    const streamingUrl = apiConfig.apiUrl.replace(
      ":generateContent",
      ":streamGenerateContent"
    );
    return await callGeminiStreamingAPI(
      streamingUrl,
      apiConfig.apiKey,
      requestBody,
      onChunk
  ...
  try {
    console.log(
      ...
      `${apiConfig.apiUrl}?key=${apiConfig.apiKey.substring(0, 10)}...`
    );
    console.log("Request body:", JSON.stringify(requestBody, null, 2));

    const response = await fetch(
      `${apiConfig.apiUrl}?key=${apiConfig.apiKey}`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        throw new Error(
          `APIリクエストエラー: リクエスト形式が無効です (${response.status})`
        );
      } else if (response.status === 401) {
        throw new Error(`認証エラー: APIキーが無効です (${response.status})`);
      } else if (response.status === 403) {
        throw new Error(
          `権限エラー: APIアクセスが拒否されました (${response.status})`
        );
      } else if (response.status === 404) {
        throw new Error(
          `エンドポイントエラー: APIエンドポイントが見つかりません (${response.status})`
        );
      } else {
        throw new Error(
    ...
    if (data.promptFeedback && data.promptFeedback.blockReason) {
      throw new Error(
        `コンテンツフィルターによりブロックされました: ${data.promptFeedback.blockReason}`
      );
    }
    ...
    if (!data.candidates || data.candidates.length === 0) {
      ...
      throw new Error(
        "AIからの応答が生成されませんでした。別のテキストでお試しください。"
      );
    }
    ...
    if (
      ...
      throw new Error(
        "予期しないレスポンス形式です: " + JSON.stringify(candidate)
      );
    }
  } catch (error) {
...
async function callGeminiStreamingAPI(url, apiKey, requestBody, onChunk) {
  try {
    console.log(
      "Making streaming API request to:",
      `${url}?key=${apiKey.substring(0, 10)}...`
    );
...
async function handleExplainText(text, context) {
  ...
  try {
    ...
    if (!apiConfig.apiKey) {
      throw new Error(
        "Gemini API キーが設定されていません。拡張機能のポップアップで設定してください。"
      );
    }

    if (!apiConfig.apiUrl) {
      throw new Error(
        "APIリクエストURLが設定されていません。拡張機能のポップアップで設定してください。"
      );
    }
